...
const CONTENT = {
  ...
  de: {
    ...
    aboutStory: [
      { icon:"🎓", heading:"Universität Hintergrund",         body:"Ich habe Betriebswirtschaftslehre studiert — doch Technologie war schon immer meine wahre Leidenschaft. Mit 14 Jahren lernte ich Computer-Grafikdesign, mit 16 brachte ich mir selbst das Programmieren bei. Parallel zu meinem Studium, angetrieben von tiefer Neugier für digitale Systeme, absolvierte ich intensive Programme wie den AYA Backend- und den ALT Africa Software-Engineering-Bootcamp." },
...
const LANGUAGES = {
  en: [
    { name: "English", level: "Native / Mother Tongue", flag: "🇬🇧" },
    { name: "German", level: "C1 — Advanced ", flag: "🇩🇪" },
  ],
  de: [
    { name: "Englisch", level: "Muttersprache", flag: "🇬🇧" },
    { name: "Deutsch", level: "C1 — Fortgeschritten ", flag: "🇩🇪" },
  ],
};
  ...
  return (
    <>
      <motion.div animate={{ x:pos.x-6,  y:pos.y-6,  scale:big?0:1   }} transition={{ type:"spring", stiffness:800, damping:40 }}
        style={{ position:"fixed", top:0, left:0, width:12,  height:12,  borderRadius:"50%", background:C.accent, pointerEvents:"none", zIndex:9999 }} />
...
/* ─── BUTTONS ─── */
function SolidBtn({ href, children, target, fullWidth, onClick }) {
  const [h, setH] = useState(false);
  const isSmoothScroll = href === "#contact" && onClick;
  const handleClick = (e) => {
    if (isSmoothScroll) {
      e.preventDefault();
      onClick(e);
  ...
  return (
    <a href={href} target={target} rel={target?"noreferrer":undefined} onClick={handleClick}
...
function EmailForm() {
  ...
  const handleSubmit = (e) => {
    ...
    window.location.href = `mailto:adeyemi504@gmail.com?subject=${encodeURIComponent(subject)}&body=${body}`;
    setStatus("sent");
    ...
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
        <input
          ...
          style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: "0.7rem 1rem", fontSize: 13, fontFamily: "'Outfit', sans-serif" }}
        />
        <input
      ...
      <textarea
        ...
        style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: "0.7rem 1rem", fontSize: 13, fontFamily: "'Outfit', sans-serif", resize: "vertical" }}
      />
      <SolidBtn type="submit" fullWidth>Send Message ✉</SolidBtn>
      {status === "sent" && <p style={{ fontSize: 12, color: "#22C55E", textAlign: "center", marginTop: "-0.5rem" }}>✓ Message opened in your email client</p>}
    </form>
  );
}
...
export default function Portfolio() {
  ...
  return (
    <>
          ...
          <div style={{ display:"grid", gridTemplateColumns:isMobile||isTablet?"1fr":"1fr 300px", gap:isMobile?"2.5rem":"4rem", alignItems:"center" }}>
            <motion.div initial={{ opacity:0, x:-30 }} animate={{ opacity:1, x:0 }} transition={{ duration:.7, ease:[.16,1,.3,1] }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:".65rem" }}>
                ...
                <SolidBtn href="#contact" onClick={scrollToContact}>{t.btnContact}</SolidBtn>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              style={{ position:"relative", display:"flex", justifyContent:"center",
                       order:isMobile?-1:0, marginBottom:isMobile?"0":"0" }}>
              ...
              <img src="/me.png" alt="Christopher Adeyemi"
                style={{ position:"relative", zIndex:1, width: isMobile?"65%":"100%", maxWidth: isMobile?220:280, display:"block",
                         objectFit:"contain", objectPosition:"top", filter:"drop-shadow(0 20px 40px rgba(242,100,25,.15))" }} />
              {!isMobile && (
                <>
      ...
      <section style={{ padding:`0 ${PAD} 5rem`, background:C.surface }}>
        <div style={{ maxWidth:MAXW, margin:"0 auto", paddingTop:"4rem" }}>
          ...
          >
            <div style={{ padding: isMobile ? "2rem" : "2.5rem", textAlign: "center" }}>
              <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:11, color:C.accent, letterSpacing:".16em", textTransform:"uppercase", marginBottom:".6rem" }}>{t.languagesLabel}</p>
              <h2 style={{ fontFamily:"'Fraunces',serif", fontWeight:900, fontSize:"clamp(28px,4vw,40px)", color:C.text, letterSpacing:"-.025em", marginBottom:"2rem" }}>{t.languagesTitle}</h2>
              <div style={{ display: "flex", justifyContent: "center", gap: isMobile ? "1.5rem" : "3rem", flexWrap: "wrap" }}>
                {languages.map((langItem) => (
                  <div key={langItem.name} style={{ textAlign: "center", minWidth: "140px" }}>
                    <div style={{ fontSize: "2.5rem", marginBottom: "0.5rem" }}>{langItem.flag}</div>
                    <p style={{ fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:16, color:C.text }}>{langItem.name}</p>
                    <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:11, color:C.accent, marginTop:"0.25rem" }}>{langItem.level}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section id="skills" style={{ padding:`0 ${PAD} 5rem` }}>
        <div style={{ maxWidth:MAXW, margin:"0 auto" }}>
          <motion.div initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:.45 }}>
      ...
      <section id="projects" style={{ padding:`5rem ${PAD}`, background:C.surface }}>
        <div style={{ maxWidth:MAXW, margin:"0 auto" }}>
          <motion.div initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:.45 }}>
      ...
      <section id="contact" style={{ padding:`5rem ${PAD}` }}>
        <div style={{ maxWidth:MAXW, margin:"0 auto" }}>
          <motion.div
            initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }}
          ...
          >
            <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr" }}>
              <div style={{ padding:isMobile?"2rem":"3rem" }}>
                ...
                <p style={{ fontSize:14, color:C.sub, lineHeight:1.85, marginBottom:"2rem" }}>{t.contactPara}</p>
                <EmailForm />
              </div>
